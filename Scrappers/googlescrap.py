...
supabase: Client = (
    create_client(SUPABASE_URL, SUPABASE_KEY) if SAVE_TO_SUPABASE and SUPABASE_URL and SUPABASE_KEY else None
)
...
def get_requested_loc_url(user_id: str, loc_id: str) -> str:
    ...
    if not supabase:
        print("[SUPABASE] Brak klienta Supabase.")
        return ""
    try:
...
def upload_to_supabase(raw_results: list, user_id: str, loc_id: str):
    if not supabase:
        print("[SUPABASE] Klient niewskazany. Pomijam zapis.")
        return
    if not raw_results:
...
async def run_google_scraper(location_id: str, user_id: str = None):
    """Główna funkcja wywoływana z zewnątrz dla konkretnej lokalizacji."""
    print(f"[SCRAPER] Rozpoczynam pobieranie dla location_id: {location_id}")
    
    maps_url = get_requested_loc_url(user_id, location_id)

    if not maps_url:
        print(f"[ERROR] Brak prawidłowego URL dla location_id: {location_id}. Przerywam.")
        return

    async with async_playwright() as p:
        browser = await p.chromium.launch(
            headless=True, slow_mo=500 if DEBUG_MODE else 0
        )
